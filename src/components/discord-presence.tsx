...
export default function DiscordPresence({ userId }: { userId: string }) {
    ...
    const hasActivity = presence.activities.some((activity: { type: number } ) => activity.type === 0);
    if (!hasActivity) {
        return null;
    }
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between mb-4">
                        <div className="flex items-center space-x-4 mb-4 sm:mb-0">
                            <div className="relative">
                                ...
                                <span className={`absolute bottom-0 right-0 w-4 h-4 rounded-full ${getStatusColor(presence.discord_status)} border-2 border-background`}></span>
                            </div>
                            <div>
                                <p className="font-medium text-lg">{presence.discord_user.username}</p>
                            </div>
                        </div>
                        <Button
                            asChild
                        ...
                        >
                            ...
                            <div className="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">{presence.spotify.artist}</p>
                                    <p className="text-xs text-muted-foreground">{presence.spotify.album}</p>
                                </div>
                            </div>
                            <div className="flex items-center mt-2 text-xs text-muted-foreground">
                                <Clock className="h-3 w-3 mr-1" />
                                <span>{formatTime(presence.spotify.timestamps.start)} - {formatTime(presence.spotify.timestamps.end)}</span>
                            </div>
                        </motion.div>
                    )}

                    {presence.activities.filter((activity) => activity.type !== 2).map((activity, index) => (
                        <motion.div
                            key={index}
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
